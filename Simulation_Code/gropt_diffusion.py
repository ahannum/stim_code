...
def optimize_diffusion_waveform_minTE(T_90, T_180, T_readout,
                                      TE_min, TE_max, dt, b_target, smax=200,T_180_center = 2.5e-3,
                                      pns_max=0.75, cns_max=0.75, gmax=0.2,custom_vec = False,
                                      safe_params=None, safe_params_cardiac=None,
                                      tol=1e-6, max_iter=200, mmt = 0, TE_start = None,
                                        pns_vec_dict = None,T_readout_init = None, iteration_logging = False):
    """
    Find the minimum TE that produces a waveform with a specified b-value.
    Uses SETVAL mode to enforce exact b-value.
    """
    ...
    while high - low > tol and iter_count < max_iter:
        ...
        #print('input parameters = {},{},{},{},{}'.format(T_90, T_180, T_readout, TE, dt)    )
        ...
        if custom_vec:
            ...
            pns_x, __, __  = adjust_stimulus_vector_keep_pulses_piecewise(
                    pns_vec_dict['safe_gx'], dt, gparams.N, T_90, T_180,
                    T_180_center, pns_vec_dict['TE'],  T_readout, TE_new = TE, stim_dic = pns_vec_dict, T_readout_init = T_readout_init)

            pns_y, __, __  =  adjust_stimulus_vector_keep_pulses_piecewise(
                    pns_vec_dict['safe_gy'], dt, gparams.N, T_90, T_180,
                    T_180_center, pns_vec_dict['TE'],  T_readout, TE_new = TE, stim_dic = pns_vec_dict, T_readout_init = T_readout_init)

            pns_z, __, __  =  adjust_stimulus_vector_keep_pulses_piecewise(
                    pns_vec_dict['safe_gz'], dt, gparams.N, T_90, T_180,
                    T_180_center, pns_vec_dict['TE'],  T_readout, TE_new = TE, stim_dic = pns_vec_dict, T_readout_init = T_readout_init)

            if safe_params_cardiac is not None:
                cns_x, __, __  = adjust_stimulus_vector_keep_pulses_piecewise(
                        pns_vec_dict['safe_cardiac_gx'], dt, gparams.N, T_90, T_180,
                        T_180_center, pns_vec_dict['TE'],  T_readout, TE_new = TE, stim_dic = pns_vec_dict, T_readout_init = T_readout_init)

                cns_y, __, __  =  adjust_stimulus_vector_keep_pulses_piecewise(
                        pns_vec_dict['safe_cardiac_gy'], dt, gparams.N, T_90, T_180,
                        T_180_center, pns_vec_dict['TE'],  T_readout, TE_new = TE, stim_dic = pns_vec_dict, T_readout_init = T_readout_init )

                cns_z, __, __  =  adjust_stimulus_vector_keep_pulses_piecewise(
                        pns_vec_dict['safe_cardiac_gz'], dt, gparams.N, T_90, T_180,
                        T_180_center, pns_vec_dict['TE'],  T_readout, TE_new = TE, stim_dic = pns_vec_dict, T_readout_init = T_readout_init )
            ...
            stim_vec_pns = [stim_vec_pns, stim_vec_pns, stim_vec_pns    ]
            stim_vec_cns = [stim_vec_cns, stim_vec_cns, stim_vec_cns    ]
        ...
        gparams.add_bvalue(b_target, mode='setval')  # exact b-value
        # SAFE constraints
        if safe_params is not None:
            for axis in range(3):
                gparams.add_SAFE_vec(stim_vec_pns[axis], safe_params=safe_params, new_first_axis=axis)
        if safe_params_cardiac is not None:
            for axis in range(3):
                gparams.add_SAFE_vec(stim_vec_cns[axis], safe_params=safe_params_cardiac, new_first_axis=axis)
        ...
        if final_good:
            # Keep the smallest TE that works

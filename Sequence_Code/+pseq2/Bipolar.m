classdef Bipolar
    %Bipolar Class for generating Bipolar diffusion gradients and computing b-value
    ...
    methods
        function obj = Bipolar(target_bval, delayTE1, delayTE2, rf180, gz180, rf90_duration, rf90_centerInclDelay, rf180_duration, gradRasterTime, diffGradMax, maxSlew, lims)
            % Constructor for Bipolar diffusion gradient generator
            if target_bval == 0 
            ...
            fprintf('Bipolar gradient prepared: Gmax = %.3f mT/m | Initial b = %.1f s/mm^2 | Final b = %.1f s/mm^2\n', obj.gAmplitude , b_initial, b_actual);
    ...
    methods (Static)
        ...
        function [bval, dummy_seq, gDiff, delayTE1,delayTE2,adjust_time] = computeMaxBValue(delayTE1, delayTE2, ...
            ...
            g_interp(isnan(g_interp)) = 0;
            g_array_Tpm = mr.convert(g_interp, 'Hz/m', 'mT/m') * 1e-3;
            bval = pseq2.Bipolar.get_bvalue(g_array_Tpm, dt);

             % Compute Moments
            moments = pseq2.Bipolar.get_moments(g_array_Tpm', dt,0);
        ...
        %   bval                 - Computed b-value (in s/mm²)
            ...
            dummy_seq = mr.Sequence(lims);
            dummy_seq.addBlock(mr.makeDelay(t_90));
            dummy_seq.addBlock(delayTE1,gDiff);
            dummy_seq.addBlock(mr.makeDelay(t_180));
            dummy_seq.addBlock(mr.makeDelay(delayTE2), mr.scaleGrad(gDiff, -1));
        ...
        % Compute b-value
        bval = pseq2.Bipolar.get_bvalue(g_array_Tpm', dt);  % units: s/mm²

classdef Monopolar
    %MONOPOLAR Class for generating monopolar diffusion gradients and computing b-value
    ...
    methods
        function obj = Monopolar(target_bval, delayTE1, delayTE2, rf180, gz180, rf90_duration, rf90_centerInclDelay, rf180_duration, gradRasterTime, diffGradMax, maxSlew, lims)
            % Constructor for Monopolar diffusion gradient generator
            if target_bval == 0 
                ...
                dt = gradRasterTime;
                dummy_seq = obj.makeDummySeq(gDiff, delayTE1, delayTE2, rf90_duration, rf90_centerInclDelay, rf180_duration,lims);
            ...
            fprintf('Monopolar gradient prepared: Gmax = %.3f mT/m | Initial b = %.1f s/mm^2 | Final b = %.1f s/mm^2\n', obj.gAmplitude , b_initial, b_actual);
            
        end

        function dummy_seq = makeDummySeq(obj, gDiff, delayTE1, delayTE2, rf90_duration, rf90_centerInclDelay, rf180_duration,lims)
    ...
    methods (Static)
        ...
        function [bval, dummy_seq, gDiff, delayTE1,delayTE2,adjust_time] = computeMaxBValue(delayTE1, delayTE2, ...
            ...
            small_delta = delayTE2 - gr;
            flat_time = small_delta-gr;

            if flat_time < 0
                adjust_time = abs(flat_time);   % How much we need to absorb
                flat_time = 0;
        ...
        %   bval                 - Computed b-value (in s/mm²)
            ...
            % Compute Moments
            moments = pseq2.Bipolar.get_moments(g_array_Tpm', dt,1);
        end
        ...
        % Compute b-value
        bval = pseq2.Monopolar.get_bvalue(g_array_Tpm', dt);  % units: s/mm²

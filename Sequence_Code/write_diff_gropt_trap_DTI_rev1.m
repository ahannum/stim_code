...
target_bval = 0; % If scaling down to 1000 for DTI

gropt_mode = 1; % Load GrOpt waveforms instead of Trap
Nslices = 31; %Number of slices
...
if gmax == 200
    ...
    % set the slew to 90 for the DTI
     ...
     sys_crusher = mr.opts('MaxGrad',45,'GradUnit','mT/m',...
        'MaxSlew',90,'SlewUnit','T/m/s',...
        'rfRingdownTime', 10e-6, 'rfDeadtime', 100e-6, 'B0', 2.89);  % 50/130
    % set the slew to 90 for the DTI
...
crusher_d=0.95e-3; % old amplitude 19.05*42.58*10e2
amp = 19.05*42.58*10e2;

gz180_crusher_1=mr.makeTrapezoid('z',sys_crusher,'Amplitude',amp,'Duration',crusher_d); % that's what we used for Siemens
gz180_crusher_2=mr.makeTrapezoid('y',sys_crusher,'Amplitude',amp,'Duration',crusher_d); % that's what we used for Siemens
gz180_crusher_3=mr.makeTrapezoid('x',sys_crusher,'Amplitude',amp,'Duration',crusher_d); % that's what we used for Siemens

%gz180_crusher_1.amplitude = 0;
%gz180_crusher_2.amplitude = 0;
%gz180_crusher_3.amplitude = 0;
...
%%
% Example: gradient area function
A = gz180_crusher_1.area; % returns Hz/m
voxel_m = 0.002;     % voxel size in meters, e.g., 2 mm

% Phase across voxel in radians
phi_voxel = 2 * pi * A * voxel_m;

% Phase in multiples of 4pi
N_4pi = phi_voxel / (4 * pi); % or equivalently: N_4pi = (A * voxel_m)/2

fprintf('Phase dispersion: %.2f rad\n', phi_voxel);
fprintf('Phase dispersion in multiples of 4pi: %.2f\n', N_4pi);
%%
